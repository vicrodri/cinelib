  ...
  return (
    <main>
      <section className="flex justify-around flex-wrap py-5">
        <div className="maw-w-sm">
          <img src={image} alt={movie.title} className="rounded"></img>
        </div>
        <div className="max-21-2xl text-gray-700 text-lg dark:text-white">
          ...
          <p className="my-7 flex flex-wrap gap-2">
            {movie.genres
              ? movie.genres.map((genre: Genre) => (
                  <span
                    key={genre.id}
                    className="mr-2 border border-gray-200 rounded dark:border-grey-600 p-2"
                  >
                    {genre.name}
                  </span>
                ))
