...
import { setItem, getItem } from "../util/Utils";

export const Header: FC = () => {
  const localDarkMode = getItem<boolean>("darkMode");
  const navigate = useNavigate();
  const [hidden, setHidden] = useState<boolean>(true);
  const [darkMode, setDarkMode] = useState<boolean>(
    localDarkMode ? localDarkMode : false
  );
  ...
  useEffect(() => {
    setItem<boolean>("darkMode", darkMode);
  ...
  const submitHandler = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const searchBox:HTMLInputElement = event.currentTarget.search as HTMLInputElement
    const queryTerm: string = searchBox.value;
        ...
        <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
          ...
          <div id="mobile-nav" className="flex md:order-2">
            ...
            <div className={`relative hidden md:block`}>
              <div className="absolute inset-y-0 start-3 flex items-center ps-1 pointer-events-none">
                <svg
